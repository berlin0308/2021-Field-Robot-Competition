...
int ledC_R=22, ledC_G=24, ledC_B=26;  // left-rear light, digital pins
...
// pins 待調整
Ultrasonic ultrasonicD(12,13); // Downward ultra - Trig 12, Echo 13
  ...
  if (Serial.available()) {
    ...
    if(sender=='1'){   // the signal is from python
     ...
     if(state=='3'){// 乙 CD關
        ...
        distanceD = ultrasonicD.read(); // 不加參數就是 CM, INC 英吋
        lcd.clear();
        lcd.setCursor(0, 0);  //設定游標位置 (字,行)
